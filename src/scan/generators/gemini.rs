...
impl IgnoreGenerator for GeminiGenerator {
    fn tool_name(&self) -> &str {
        "Gemini CLI"
    }

    fn is_installed(&self, project_dir: &Path) -> bool {
        project_dir.join(".gemini").exists()
            || project_dir.join(".geminiignore").exists()
    }

    fn generate(&self, patterns: &[String], project_dir: &Path) -> Result<GenerateResult> {
        let ignore_path = project_dir.join(".geminiignore");
